<template>
  ...
  >
      ...
      <el-tab-pane class="first" label="图库" name="first">
        ...
        <div class="slot-wrap">
          <div v-for="(item, index) in params.newArr" :key="index" class="adslot defult">
            <!-- 如果是海清设备待下发的图片没有删除功能  -->
            <i v-if="item.url && (dataForm.deviceFactory !== 'hq')" class="el-icon-error" @click="delOneHandle(item.id)" />
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <!-- <el-button v-if="activeName === 'first'" type="primary" :disabled="!!!list.filter(item => item.isSelect).length" @click="imgListHandleRemove()">删除</el-button> -->
    </span>
  </el-dialog>
</template>

<script>
import { imglist, imglistDelete, selectListDelete, selectList, batchSave, issue } from '@/api/project/device'
export default {
  data() {
    return {
