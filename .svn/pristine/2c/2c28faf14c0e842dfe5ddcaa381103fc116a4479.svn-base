  ...
  <div class="device">
    <el-form :inline="true" :model="dataForm">
      ...
      <el-form-item prop="deviceFactory">
        <el-select
          v-model.trim="dataForm.deviceFactory"
          clearable
          placeholder="请选择设备厂家"
        >
      ...
      <div class="search-btn-style">
        <el-button
          v-permit="'bqjyDevice_btn_add'"
          type="primary"
          @click="addOrUpdateHandle({})"
        >新增</el-button>
        <el-button
          v-permit="'bqjyDevice_btn_carried'"
          type="primary"
          @click="detailHandle()"
        >设备执行明细</el-button>
        <el-button
          v-permit="'bqjyDevice_btn_setsystime'"
          type="primary"
          :disabled="dataListSelections.length <= 0"
          @click="setsystimeHandle()"
        >设置系统时间</el-button>
        <el-button
          v-permit="'bqjyDevice_btn_upgrade'"
          type="primary"
          :disabled="dataListSelections.length <= 0"
          @click="upgradeModal()"
        >设备升级</el-button>
        <el-button
          v-permit="'bqjyDevice_btn_restart'"
          type="primary"
          :disabled="dataListSelections.length <= 0"
          @click="restartHandle()"
        >设备重启</el-button>
        <el-button
          v-permit="'bqjyDevice_btn_delete'"
          type="danger"
          :disabled="dataListSelections.length <= 0"
          @click="deleteHandle()"
        >批量删除</el-button>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span
            :style="{ color: scope.row.connectTimeStyle ? 'red' : '#67C23A' }"
          >{{ scope.row.connectDate }}</span>
        </template>
      </el-table-column>
      <el-table-column
        v-if="
          basePermit('bqjyDevice_btn_update') ||
            basePermit('bqjyDevice_btn_delete')
        "
        header-align="center"
        align="center"
        width="100"
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-permit="'bqjyDevice_btn_update'"
            type="text"
            size="small"
    ...
    <!-- 设备执行明细 -->
    <device-detail
      v-if="detailDeviceVisible"
      ref="detailDevice"
      @refreshDataList="getDataList"
    />
    <!-- 设备升级 -->
    <device-upgrade
      v-if="upgradeDeviceVisible"
      ref="upgradeDevice"
      @refreshDataList="getDataList"
    />
    <!-- 同步人员 -->
...
export default {
  ...
  methods: {
      ...
      getList({
        ...
        if (data && data.code === 1000) {
          this.dataList = data.result.records.map(item => {
            item.connectDate = item.connectTime
              ? parseTime(item.connectTime, '{y}-{m}-{d} {h}:{i}:{s}')
              : ''
    ...
    // 双击table
    rowDblclick(row) {
      if (this.basePermit('bqjyDevice_btn_update')) this.addOrUpdateHandle(row)
    },
    basePermit(e) {
    ...
    subscribeHandle(id) {
      ...
      this.$confirm(`您确定进行订阅认证操作吗?`, '提示', {
        ...
        .then(() => {
          subscribe({ deviceNoS: ids.length ? ids.toString() : '' }).then(
            data => {
              if (data && data.code === 1000) {
                this.$message({
                  message: '操作成功',
    ...
    upgradeHandle(id) {
      ...
      this.$confirm(`您确定进行设备升级操作吗?`, '提示', {
        ...
        .then(() => {
          upgrade({ deviceNoS: ids.length ? ids.toString() : '' }).then(
            data => {
              if (data && data.code === 1000) {
                this.$message({
                  message: '操作成功',
    ...
    restartHandle(id) {
      ...
      this.$confirm(`您确定进行设备重启操作吗?`, '提示', {
        ...
        .then(() => {
          rebootdevice({ deviceNoS: ids.length ? ids.toString() : '' }).then(
            data => {
              if (data && data.code === 1000) {
                this.$message({
                  message: '操作成功',
