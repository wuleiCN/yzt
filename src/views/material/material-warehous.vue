...
<script>
import ProjectModal from './warehous-add-or-update.vue'
import { getList, deleteById } from '@/api/material/materialWarehous'
...
export default {
  ...
  mounted() {
    this.getDataList()
    this.exportUrl = `/materialInHouse/export?token=${this.token}&page=${this.pageIndex}&rows=9999`
  },
  methods: {
    // 获取数据列表
    getDataList() {
      ...
      getList({
        ...
        if (res.result && res.code === 1000) {
          ...
          this.dataList = res.result.records.map(item => {
            item.inHouseTime = item.inHouseTime
              ? parseTime(item.inHouseTime, '{y}-{m}-{d} {h}:{i}:{s}')
              : ''
            return item
          })
