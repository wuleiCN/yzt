<template>
  <el-dialog
    custom-class="zhgd-electric-modal"
    :title="!dataForm.id ? '新增用电设备' : '修改用电设备'"
    ...
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="120px" @keyup.enter.native="dataFormSubmit()">
      ...
      <div class="base-info">
        <el-form-item label="电表名称" prop="name">
          <el-input v-model.trim="dataForm.name" placeholder="电表名称" />
        </el-form-item>
        ...
        <el-form-item label="电表类别" prop="type">
          <el-select v-model.trim="dataForm.type" style="width:100%" placeholder="请选择电表类别">
        ...
        <el-form-item label="费率" prop="caliber">
          <el-select v-model.trim="dataForm.caliber" style="width:100%" placeholder="请选择费率">
        ...
        <el-form-item label="所属电表" prop="waterFlowmeter">
          <el-select v-model.trim="dataForm.waterFlowmeter" style="width:100%" placeholder="请选择所属电表">
            <el-option label="电表1" :value="1"> 电表1 </el-option>
            <el-option label="电表2" :value="2"> 电表2 </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="CT(电流)" prop="multiplyingPower">
          <el-input v-model.number="dataForm.multiplyingPower" placeholder="CT(电流)" />
        </el-form-item>
        <el-form-item label="PT(电压)" prop="multiplyingPower">
          <el-input v-model.number="dataForm.multiplyingPower" placeholder="PT(电压)" />
        </el-form-item>
      ...
      <div class="communication-info">
        <el-form-item label="通讯方案" prop="communicationScheme">
          <el-select v-model.trim="dataForm.communicationScheme" style="width:100%" placeholder="请选择通讯方案">
            <el-option label="NB-IOT常德电表" :value="1"> NB-IOT常德电表 </el-option>
            <el-option label="NB-IOT常德电表" :value="2"> NB-IOT常德电表 </el-option>
          </el-select>
        </el-form-item>
...
<script>
import { saveOrUpdateElectric } from '@/api-zhgd/electric-meter'
  ...
  methods: {
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          saveOrUpdateElectric({ ...this.dataForm }).then((data) => {
            if (data && data.code === 1000) {
              this.$message({
                message: '操作成功',
                type: 'success',
