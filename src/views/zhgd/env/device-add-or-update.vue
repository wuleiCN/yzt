<template>
  <el-dialog
    ...
    :title="!dataForm.id ? '新增扬尘设备' : '修改扬尘设备'"
  ...
  >
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="120px" @keyup.enter.native="dataFormSubmit()">
      ...
      <el-form-item label="安装位置" prop="installAddress">
        ...
        <!-- <el-input v-model.trim="dataForm.installAddress" placeholder="安装位置" /> -->
      </el-form-item>
      ...
      <el-form-item label="是否喷淋设备:" prop="type">
...
<script>
import { save, update } from '@/api-zhgd/env-device'
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        type: [
          { required: true, message: '是否喷淋设备不能为空', trigger: 'blur' }
        ]
      }
    }
  },
