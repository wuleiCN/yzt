<template>
  <el-dialog
    custom-class="elevator-bind-modal"
    title="绑定升降机黑匣子"
  ...
  >
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="150px" @keyup.enter.native="dataFormSubmit()">
      ...
      <el-form-item label="经度：" :disabled="true" prop="latitude">
        <el-input v-model.trim="dataForm.latitude" :disabled="true" placeholder="经度" />
      </el-form-item>
      <el-form-item label="纬度：" :disabled="true" prop="longitude">
...
<script>
import { update, view1 } from '@/api-zhgd/zhgd-elevator'
...
export default {
  ...
  methods: {
    ...
    init(row) {
      ...
      this.$nextTick(() => {
        if (row.id) {
          view1({ elevatorId: row.id }).then(data => {
            if (data && data.code === 1000) {
              this.disabled = data.result && data.result.hxzId
              this.dataForm = data.result || { elevatorId: row.id }
            }
          })
        }
