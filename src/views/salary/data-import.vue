<template>
  <el-dialog
    custom-class="salary-import-modal"
    title="导入"
  ...
  >
    <el-form ref="dataForm" :model="dataForm" label-width="100px">
      ...
      <el-form-item label="模板下载:">
        <div><a target="_blank" class="temp-name" :href="$http.staticUrl('/薪资输入项导入-默认薪资方案.xlsx')">1、薪资输入项导入-默认薪资方案</a></div>
        <div>
          <a target="_blank" class="temp-name" :href="$http.staticUrl('/薪资输入项导入-创新薪资方案.xlsx')">2、薪资输入项导入-创新薪资方案</a>
        </div>
      </el-form-item>
      <el-form-item label="导入模板:">
        <my-upload ref="myUpload" v-permit="'project_person_btn_import'" :is-show="true" :limit="'doc'" :data="{month: dataForm.month}" :title="'点击上传'" :action="'/salary/excelImport'" @getfileList="getFileData" />
      </el-form-item>
