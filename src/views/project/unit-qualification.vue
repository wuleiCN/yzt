    ...
    >
      <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="100px">
        <el-form-item label="资质类型" prop="qualificationType">
          <el-select v-model="dataForm.qualificationType" filterable style="width:100%" placeholder="请选择资质类型">
        ...
        <el-form-item label="资质名称" prop="qualificationName">
          <el-input v-model.trim="dataForm.qualificationName" placeholder="资质名称" />
        </el-form-item>
        <el-form-item label="资质编号" prop="qualificationId">
          <el-input v-model.trim="dataForm.qualificationId" placeholder="资质编号" />
        </el-form-item>
        ...
        <el-form-item label="资质等级" prop="qualificationLevel">
          <el-select v-model="dataForm.qualificationLevel" filterable style="width:100%" placeholder="请选择资质等级">
...
<script>
import { conQualList, conQualSaveOrUpdate, conQualDel } from '@/api/project/unit'
...
export default {
  ...
  methods: {
    // 下拉资质类型
    getOptionList() {
      optionList({ category: 'QUALIFICATION_TYPE' }).then(data => {
        this.types = data.result
      })
    },
    // 下拉资质等级类型
    getOptionList1() {
      optionList({ category: 'QUALIFICATION_LEVEL' }).then(data => {
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          conQualSaveOrUpdate({ ...this.dataForm, suid: this.suid }).then((data) => {
            if (data && data.code === 1000) {
              this.$message({
                message: '操作成功',
                type: 'success',
