import request from '@/utils/request'

export function getList(data) {
  return request({
    url: '/device/attlist',
    method: 'post',
    data
  })
}

export function saveOrUpdate(data) {
  return request({
    url: '/device/saveorupdate',
    method: 'post',
    data
  })
}

export function del(data) {
  return request({
    url: '/device/del',
    method: 'post',
    data
  })
}
// 设备执行列表
export function attcmdinfoList(data) {
  return request({
    url: '/attcmdinfo/list',
    method: 'post',
    data
  })
}
// 设置系统时间
export function setsystime(data) {
  return request({
    url: '/device/setsystime',
    method: 'post',
    data
  })
}
// 设置远程开门
export function openDoor(data) {
  return request({
    url: '/device/open',
    method: 'post',
    data
  })
}
// 接入服务
export function subscribe(data) {
  return request({
    url: '/device/accessService',
    method: 'post',
    data
  })
}
// 设备升级
export function upgrade(data) {
  return request({
    url: '/device/upgrade',
    method: 'post',
    data
  })
}
// 手撕鸡设备升级
export function upssj(data) {
  return request({
    url: '/device/upSCJ',
    method: 'post',
    data
  })
}
// 设备重启
export function rebootdevice(data) {
  return request({
    url: '/device/rebootdevice',
    method: 'post',
    data
  })
}
//
export function hkSetFree(data) {
  return request({
    url: '/device/hkSetFree',
    method: 'post',
    data
  })
}
// 同步人员
export function synworker(data) {
  return request({
    url: '/device/synworker',
    method: 'post',
    data
  })
}
// 图库列表
export function imglist(data) {
  return request({
    url: '/yztGallery/list',
    method: 'post',
    data
  })
}
// 图库列表批量删除
export function imglistDelete(data) {
  return request({
    url: '/yztGallery/delete',
    method: 'post',
    data
  })
}
// 已下发图片删除
export function selectListDelete(data) {
  return request({
    url: '/yztDeviceAd/delete',
    method: 'post',
    data
  })
}
// 图库已下发图片列表列表
export function selectList(data) {
  return request({
    url: '/yztDeviceAd/list',
    method: 'post',
    data
  })
}
// 新图上传保存
export function batchSave(data) {
  return request({
    url: '/yztGallery/batchSave',
    method: 'post',
    data
  })
}
// 替换已下发图片
export function issue(data) {
  return request({
    url: '/yztDeviceAd/issue',
    method: 'post',
    data
  })
}
// 所属项目下拉
export function projectSelectList(data) {
  return request({
    url: '/projects/optionList',
    method: 'post',
    data
  })
}
// 所属项目下拉
export function deviceOptionList(data) {
  return request({
    url: '/productPurchaseDetail/optionList',
    method: 'post',
    data
  })
}
